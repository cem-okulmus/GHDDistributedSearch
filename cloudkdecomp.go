...
// Computing GHDs with a distributed search for separators, first prototype
...
func output(algorithm string, decomp lib.Decomp, times []labelTime, graph lib.Graph, gml string, K int) {
...
func main() {
	...
	// Sorting lib.Edges to find separators faster
	if *useHeuristic > 0 {
	...
	if *typeC {
		...
		if !*bench { // be silent when benchmarking
			...
			for _, e := range reducedGraph.Edges.Slice() {
				fmt.Printf("%v %v\n", e, lib.Edge{Vertices: e.Vertices})
			}
	...
	if solver != nil {
		...
		d := time.Now().Sub(start)
		msec := d.Seconds() * float64(time.Second/time.Millisecond)
		times = append(times, labelTime{time: msec, label: "lib.Decomposition"})

		if !reflect.DeepEqual(decomp, lib.Decomp{}) || (len(ops) > 0 && parsedGraph.Edges.Len() == 0) {
			var result bool
		...
		output(solver.Name(), decomp, times, originalGraph, *gml, *width)
